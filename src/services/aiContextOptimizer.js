...
const OUTPUT_SCHEMA = {
  ...
  accion:
    "agregar|consultar|consultar_pago|nuevo_pedido|repetir_pedido|confirmar|rechazar|quitar|mantener_solo|modificar_cantidad|null",
  confianza: 0,
  producto: {
    marca: null,
    referencia: null,
...
function instruccionesPerfil(perfil) {
  ...
  const perfiles = {
    ...
    "Interpreta primero la respuesta respecto a estado.esperando y la ultima pregunta. Productos consultados o incluidos en un resumen son contexto historico, no una solicitud nueva. Solo cambia al catalogo si el mensaje actual expresa una nueva consulta o cambio de producto. Al confirmar o completar datos, deja producto y productos vacios.",
      ...
      "Para coordinar domicilio de un pedido activo, usa intencion=datos_envio, entrega.tipo=domicilio y continuarFlujo=true. El motor solicita juntos todos los datos pendientes: nombre, cedula, celular, correo, direccion completa y metodo de pago. Extrae todos los datos aportados en el mensaje o lote; no preguntes uno por uno ni vuelvas a solicitar datos conocidos. Si la respuesta es parcial, solicita juntos solo los faltantes. Una consulta general de domicilios sin compra definida no autoriza agregar productos ni pedir datos personales.",
      "Al completar datos de un pedido, continuarFlujo=true e intencion=datos_envio: el motor debe mostrar el resumen completo de productos, total, domicilio y pago. No lo reemplaces por una pregunta generica de finalizar. Ante el resumen, interpreta dejarlo asi o no agregar mas como confirmar si ese es el sentido conversacional; una pregunta o una solicitud de agregar/corregir no confirma. Un si aislado ante dos alternativas es ambiguo: aclara cual desea sin borrar el pedido. Conserva carrito, entrega y datos mientras respondes consultas intermedias.",
      ...
      "Pedir cuentas, llave o instrucciones para consignar es intencion=metodo_pago y accion=consultar_pago. No es un pedido nuevo ni otra confirmacion. Si el pedido ya esta confirmado, responde solo la consulta de pago conservando el pedido. Si tambien aporta direccion u otros datos de un pedido pendiente, extraelos para que el motor atienda ambas cosas.",
      "Consignacion o consignar como metodo de pago equivale a transferencia bancaria. Usa entrega.metodoPago=transferencia bancaria; el motor proporciona las cuentas y la llave autorizadas. Solicita los datos pendientes juntos sin exigir que el cliente responda en un solo mensaje.",
