-- Conserva siglas en FTS sin modificar productos ni precios.
-- Ejecutar despues de 005_catalog_search_rpc.sql.
begin;
...
as $$
  ...
  scored as (
    select
      p.*,
      (
        ts_rank_cd(to_tsvector('simple', p.search_text), q.ts_query) * 8
      ...
      concat_ws(
        ', ',
        case when to_tsvector('simple', p.search_text) @@ q.ts_query then 'fts' end,
    ...
    where
      q.query_text <> ''
      and (
        to_tsvector('simple', p.search_text) @@ q.ts_query
