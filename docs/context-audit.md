...
Continuidad de compra con transferencia (2026-09-18): el router distingue una solicitud de compra con preguntas de costo de una cotizacion informativa; pedir transferencia junto con un producto no sustituye la intencion de compra. La interpretacion posterior al catalogo conserva intencion, accion y datos de entrega/pago extraidos por el router. La ruta de presentacion validada usa tambien el domicilio conocido para solicitar los datos faltantes. Las respuestas del motor incluyen las cuentas automaticamente si el metodo es transferencia y aun no fueron enviadas. El humanizador separa las cuentas autorizadas del contenido comercial, genera la introduccion del producto y conserva resumen y siguiente paso operativo. Prueba integrada con segundo interprete que degrada la compra a consulta y omite entrega: dos mensajes, cuentas/llave primero, producto y todos los datos faltantes despues, sin repetir direccion/pago ni duplicar carrito. Se verifica tambien que una introduccion generada sin pregunta no suprima la pregunta de avance del motor. Suite local: 326 pruebas aprobadas, sin envio real de WhatsApp ni cambios de Supabase.

Seguimiento del registro de terminal (2026-09-18): la consulta semantica de recuperacion tambien puede contener prosa comercial y logistica. La validacion ahora utiliza la identidad estructurada del router previo al catalogo cuando sus atributos y peso estan respaldados por la consulta; si no lo estan conserva la validacion anterior, sin aceptar referencias inferidas. Una identidad completa respaldada no se combina con terminos contaminados de aclaraciones anteriores. La regresion cubre consulta larga, seguimiento de precio y carrito sin duplicar. El humanizador conserva atributos conocidos en aclaraciones y devuelve al modelo su respuesta rechazada y el motivo especifico para corregirla; registra ese motivo sin publicar datos del cliente. El registro original no permite determinar cual regla rechazo las dos respuestas. Si ambos intentos siguen siendo invalidos, el error sigue propagandose sin publicar precios inventados. Verificacion local: 325 pruebas aprobadas; sin despliegue ni cambios en Supabase.

Identidad y redaccion de productos (2026-09-18): se reprodujo la mezcla FILPO/FILPO GATOS con el catalogo local. El agrupador del validador comprobaba especie solo en el nombre y descartaba esos tokens al comparar identidad. Ahora contrasta especie estructurada (con respaldo del nombre), categoria y compatibilidad con todos los integrantes del grupo antes de unir presentaciones. No hay excepciones por marca. Las consultas de texto se validan con la consulta semantica de producto, separada de direccion, pago y domicilio; el motor conserva la interpretacion operativa. Se corrigio una prueba anterior que esperaba unir referencias de perro y gato.

Las respuestas de producto y aclaraciones pasan por redaccion de IA basada en hechos validados: no se exige copiar fichas ni encabezados. Las coincidencias inciertas reciben una pregunta para identificar el producto sin cotizar candidatos. La validacion de salida controla precios autorizados y, para un producto/presentacion exactos, referencia, peso y precio; una redaccion invalida se reintenta una vez y despues se propaga el error, sin publicar una plantilla como respaldo. Los bloques operativos de cierre y cuentas conservan su proteccion y separacion de mensajes. La tarifa de domicilio no se inventa ni se confunde con el precio del producto. Se verificaron dos redacciones con OpenAI real sin enviar WhatsApp; suite local de 324 pruebas, incluida recuperacion de contexto logistico y separacion por especie con marcas sinteticas. No se modificaron datos de Supabase.

Respuesta de consignacion en dos mensajes (2026-09-15): el motor usa el separador interno de respuestas para enviar primero cuentas/llave y despues el resumen con la pregunta de cierre, o los campos faltantes si aun no esta completo. El webhook envia las partes en orden por el mismo canal; el humanizador conserva respuestas multiples y el historial elimina el separador tecnico. Un pedido ya confirmado recibe solo las cuentas. Suite local: 319 pruebas aprobadas, incluida la ruta HTTP con envio Kapso simulado que verifica ambos mensajes y la consulta posterior al cierre.

Consulta de pago con pedido pendiente (2026-09-15): se elimina la salida que devolvia solo cuentas cuando el router omitia direccion/datosCliente. `consultar_pago` distingue primero pedido confirmado (solo instrucciones) de carrito pendiente (instrucciones mas avance del pedido). Recupera tambien datos del texto; una direccion aportada en el turno actual reemplaza la anterior y permite mostrar el resumen final si todos los campos estan completos, sin confirmar automaticamente. Si faltan campos solicita solo esos. Suite local: 318 pruebas aprobadas, incluyendo direccion mas consignacion con extraccion semantica incompleta.

Cierre con agradecimiento (2026-09-15): se reprodujo el bucle cuando el interprete devuelve el metodo de pago historico junto a una confirmacion, o clasifica «asi esta bien\ngracias» como agradecimiento. El pago repetido ya no precede a una confirmacion semantica. Con resumen pendiente, carrito y datos completos, una respuesta completa de cierre explicito admite agradecimiento adicional y se delega al motor como confirmacion; no aplica a gracias solo, si ambiguo, preguntas o solicitudes adicionales. Conserva la clave del pedido. Pruebas: lote de dos eventos, pago historico, clasificacion incorrecta y casos que no deben confirmar; suite local de 316 pruebas aprobada.

Soporte de pago despues del cierre (2026-09-15): las cuentas se incluyen tambien al confirmar datos de un pedido anterior y al confirmar el pedido si aun no se han enviado. La accion semantica `consultar_pago` atiende solicitudes explicitas de cuentas o llave, incluso si ya fueron enviadas. Sobre un pedido confirmado devuelve solamente las instrucciones de pago, conservando carrito, identificador, snapshot y banderas de confirmacion/persistencia. No reabre el cierre. Una compra nueva sigue iniciando otro pedido mediante el flujo existente. No requiere cambios de esquema en Supabase.

Actualizacion 2026-09-15: la solicitud de datos conserva todos los faltantes juntos, pero ya no exige responder «en un solo mensaje». Consignacion/consignación/consignar se normaliza a transferencia bancaria. El motor adjunta las cuentas autorizadas y la llave existentes al solicitar datos o presentar el resumen, incluido un cambio de pago en esa etapa, una sola vez por pedido. No requiere migracion SQL: usa `state.metodoPago` y `state.instruccionesPagoEnviadas`, ya persistidos. Suite local: 312 pruebas aprobadas.
...
Cierre de pedido: al completar los datos, el motor devuelve el resumen del carrito y todos los datos de domicilio disponibles, incluido el pago, seguido de «¿Deseas agregar algo más o finalizamos el pedido así?». El humanizador no puede abreviar este resumen operativo. Las intenciones de datos, pago y confirmacion con una operacion pendiente pasan por el motor incluso si el router omite continuarFlujo. Una intencion de pregunta, adicion o correccion no se confirma por contener una afirmacion. El historial y la ultima respuesta se siguen guardando junto al carrito, datos, etapa pendiente e identificador estable del pedido. La suite (304 pruebas) verifica persistencia del estado entre resumen, pregunta intermedia y confirmacion semantica.

Recopilacion conjunta de domicilio (2026-09-15): para un pedido activo, el motor solicita en un solo mensaje todos los campos pendientes (nombre, cedula, celular, correo, direccion completa y metodo de pago). El pago ya no se pregunta antes de los demas datos. Cada respuesta parcial conserva lo recibido y solicita juntos solo los faltantes; al completar datos y pago muestra el resumen y pregunta si desea agregar algo mas o finalizar, sin confirmar automaticamente. La solicitud conjunta queda protegida del humanizador. Datos y pago se delegan al motor aunque el router omita continuarFlujo, sin requerir una bandera de espera previa cuando hay un carrito sin confirmar. Una consulta general sin pedido no autoriza crear carrito ni pedir datos personales. Las pruebas cubren lotes completos, respuestas parciales, pago pendiente, transferencia, recarga de estado y proteccion frente a una pregunta individual del modelo.

Referencias con siglas: los codigos comerciales cortos se extraen antes de eliminar puntuacion o palabras funcionales; se conservan las barras (p. ej. I/D). El nombre visual debe conservar la sigla aunque junto a ella aparezca una descripcion larga. El validador exige el codigo en la referencia, y la consolidacion no fusiona por similitud referencias con codigos distintos, salvo equivalencia declarada. Una descripcion larga no obliga a que el catalogo la repita cuando ya contiene el codigo exacto. Se sigue desambiguando por marca, especie y presentacion. Verificacion de candidatos reales de Supabase con evidencia estructurada de la foto: PRO PLAN FELINE UR 1.5kg, $113.600; no PRO PLAN CAT URINARY ni la lata. No se envio WhatsApp ni se realizo nueva inferencia visual en esa comprobacion. Suite: 305 pruebas aprobadas.
